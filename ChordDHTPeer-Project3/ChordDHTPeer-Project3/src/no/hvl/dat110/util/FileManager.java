...
public class FileManager {
    ...
    public int distributeReplicastoPeers() throws RemoteException {
		...
		for(int i=counter; i < replicafiles.length; i++) {
			// for each replica, find its successor by performing findSuccessor(replica)
	    	
			NodeInterface c=chordnode.findSuccessor(replicafiles[i]);
	    	c.addKey(replicafiles[i]);
	    	c.saveFileContent(filename, c.getNodeID(), bytesOfFile, true);
	    	}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for(int i=0;i < succinfo.size(); i++) {
			...
			succinfo.add(chordnode.getFilesMetadata(c.getNodeID()));
