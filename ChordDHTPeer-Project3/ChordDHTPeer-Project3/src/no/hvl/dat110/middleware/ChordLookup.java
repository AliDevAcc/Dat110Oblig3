...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface succ=this.node;
		NodeInterface node2=succ;
		// get the successor of the node
		succ.getSuccessor();
		// get the stub for this successor (Util.getProcessStub())
		NodeInterface stubSucc=Util.getProcessStub(succ.getNodeName(), succ.getPort());
		...
		if(stubSucc!=null) {
			boolean check=Util.computeLogic(key,node2.getNodeID().add(new BigInteger("1")),
					stubSucc.getNodeID());
		if(check) {
			return succ;
		} else {
			// if logic returns false; call findHighestPredecessor(key)
			NodeInterface Hpred=findHighestPredecessor(key);
			// do return highest_pred.findSuccessor(key) - This is a recursive call until logic returns true
			return Hpred.findSuccessor(key);		
		}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i=table.size()-1; i >=0; i--) {
			...
			NodeInterface stub=Util.getProcessStub(table.get(i).getNodeName(),table.get(i).getPort());
			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. 
			//(nodeID+1 <= finger <= key-1) using the ComputeLogic
			check=Util.computeLogic(stub.getNodeID(),
					this.node.getNodeID().add(new BigInteger("1")), key.subtract(new BigInteger("1")));
			if(check) {
